...
.wrap{max-width:1400px;margin-inline:auto;padding:clamp(28px,5vh,60px) clamp(18px,3.5vw,40px) 72px}
...
.cards{display:grid;gap:16px;grid-template-columns:repeat(auto-fill,minmax(420px,1fr));
  align-items:start}
@media (max-width:900px){.cards{grid-template-columns:minmax(0,1fr)}}
/* The hero used to be one narrow column with a canyon beside it. At width the
   title and the two paragraphs sit side by side instead. */
.hero{display:grid;gap:8px 46px;align-items:end}
@media (min-width:1100px){.hero{grid-template-columns:minmax(0,1.05fr) minmax(0,1fr)}
  .hero h1{grid-row:span 2;margin-bottom:0}}
